const geojson = require('./data/ny_new_york_zip_codes_geo.min.json');
...
geometries.forEach(geometry => {
  const name = geometry.properties.ZCTA5CE10;
  const matchingObject = pResUnitList.find(item => item.zipCode === name);
  if (matchingObject) {
    geometry.properties.spending = matchingObject.spending;
  } else {geometry.properties.spending = 0}
});

const fs = require('fs'); // Import the 'fs' module to work with files

// Define the path and filename for the output file
const outputPath = './data/ny_new_york_zip_codes_geo_spending.json';
